...
var rGBCube : BissectedCube = getRGBCube("dataColors.txt")
...
fun applyOnImage(pathIn : String, pathOut: String, rGBCube : BissectedCube) {
    val outputImage = getFilteredImage(pathIn, rGBCube, tvThickness)
    saveImage(outputImage, pathOut)
}

fun getFilteredImage(pathIn : String,rGBCube : BissectedCube, tvThicknessLocal:Int, view:UserInterface?=null):BufferedImage {
    val inputImage = ImageIO.read(File(pathIn))
    ...
    for(elt in unionFindForrest) {
        ...
        if (progressIterator >= progressUpdate) {
            ...
            view?.setProgress(allProgress.toFloat()/unionFindForrest.size.toFloat())
        }
    }
    ...
    applyTVEffect(outputImage,tvThicknessLocal)

    return outputImage
}

fun saveImage(outputImage : BufferedImage, pathOut : String):Boolean {
    try {
